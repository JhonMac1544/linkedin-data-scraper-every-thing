...
function buildSuggestions(query, total = 10) {
  const base = query || 'Service';
  const items = [];
  for (let i = 0; i < total; i += 1) {
    const name = `${base} Service Category ${i + 1}`;
    items.push(
      formatSuggestion({
        endpoint: 'suggestion-service-category',
...
module.exports = async function suggestionServiceCategory(body, context = {}) {
  const { logger, defaults } = context;
  validateBody('suggestion-service-category', body);
  ...
  if (logger && logger.info) {
    logger.info(
      `suggestion-service-category for query="${query}" returned page ${pageInfo.page}/${pageInfo.totalPages} (${items.length} suggestions).`
    );
  }
