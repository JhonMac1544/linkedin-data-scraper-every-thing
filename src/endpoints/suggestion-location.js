...
function buildSuggestions(query, total = 10) {
  const base = query || 'Location';
  const items = [];
  for (let i = 0; i < total; i += 1) {
    const name = `${base} Location ${i + 1}`;
    items.push(
      formatSuggestion({
        endpoint: 'suggestion-location',
...
module.exports = async function suggestionLocation(body, context = {}) {
  const { logger, defaults } = context;
  validateBody('suggestion-location', body);
  ...
  if (logger && logger.info) {
    logger.info(
      `suggestion-location for query="${query}" returned page ${pageInfo.page}/${pageInfo.totalPages} (${items.length} suggestions).`
    );
  }
