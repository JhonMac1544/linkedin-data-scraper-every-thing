...
function buildSuggestions(query, total = 10) {
  const base = query || 'Person';
  const items = [];
  for (let i = 0; i < total; i += 1) {
    const name = `${base} Person ${i + 1}`;
    items.push(
      formatSuggestion({
        endpoint: 'suggestion-person',
...
module.exports = async function suggestionPerson(body, context = {}) {
  const { logger, defaults } = context;
  validateBody('suggestion-person', body);
  ...
  if (logger && logger.info) {
    logger.info(
      `suggestion-person for query="${query}" returned page ${pageInfo.page}/${pageInfo.totalPages} (${items.length} suggestions).`
    );
  }
